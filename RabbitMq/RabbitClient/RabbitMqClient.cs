...
namespace RabbitClient
{
    ...
    public class RabbitMqClient : IServiceBus
    {
        ...
        internal RabbitMqClient(RabbitMqServiceConfiguration config)
        {
            ...
            _connection = new Lazy<RabbitMqConnection>(() => new RabbitMqConnection(config.ConnectionConfiguration, config.ClientName), true);
            _config = config;
        ...
        /// <param name="message"></param>
        /// <param name="ttl"></param>
        private void BasicPublish(string exchangeName, string queueNameOrRoutingKey, IBusMessage message, TimeSpan? ttl = null)
        {
            using (var channel = _connection.Value.Connection.CreateModel())
            {
                ...
                lock (_lock)
                {
                    ...
                    var envelope = new ServiceBusMessageEnvelope(Guid.NewGuid(),
                        new ServiceBusMessageProperties(exchangeName, queueNameOrRoutingKey, DateTime.UtcNow, clientName),
        ...
        private IRestResponse GetResponse(string url, Method method, Dictionary<string, string> headers = null)
        {
            ...
            {
                JsonSerializer = new Serializers.JsonSerializer()
            };
